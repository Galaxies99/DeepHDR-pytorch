        ...
        if self.count == 0:
            for i, scene_dir in enumerate(self.scene_dirs):
                ...
                for j, in_LDR_path in enumerate(in_LDR_paths):
                    in_LDRs[:, :, j * c:(j + 1) * c] = get_image(in_LDR_path)
                ...
                ref_HDR = get_image(os.path.join(cur_scene_dir, ref_hdr_name))
                ...
                for j, ref_LDR_path in enumerate(ref_LDR_paths):
                    ref_LDRs[:, :, j * c:(j + 1) * c] = get_image(ref_LDR_path)
                ...
                for _h in range(0, h - self.patch_size[0] + 1, self.patch_stride):
                    for _w in range(0, w - self.patch_size[1] + 1, self.patch_stride):
                        store_patch(_h, _h + self.patch_size[0], _w, _w + self.patch_size[1], in_LDRs, in_exps,
                                    ref_HDR, ref_LDRs, ref_exps, self.patch_path, self.count)
                        self.count += 1
                if h % self.patch_size[0]:
                    for _w in range(0, w - self.patch_size[1] + 1, self.patch_stride):
                        store_patch(h - self.patch_size[0], h, _w, _w + self.patch_size[1], in_LDRs, in_exps,
                                    ref_HDR, ref_LDRs, ref_exps, self.patch_path, self.count)
                        self.count += 1
                if w % self.patch_size[1]:
                    for _h in range(0, h - self.patch_size[0] + 1, self.patch_stride):
                        store_patch(_h, _h + self.patch_size[0], w - self.patch_size[1], w, in_LDRs, in_exps,
                                    ref_HDR, ref_LDRs, ref_exps, self.patch_path, self.count)
                        self.count += 1
                if h % self.patch_size[0] and w % self.patch_size[1]:
                    store_patch(h - self.patch_size[0], h, w - self.patch_size[1], w, in_LDRs, in_exps,
                                ref_HDR, ref_LDRs, ref_exps, self.patch_path, self.count)
                    self.count += 1
